...
void AsyncNetworkClient::processQueue() {
    Request request = std::move(m_queue.front()); // Take ownership of the next request.
    m_queue.pop(); // Remove the request from the queue.
...
void AsyncNetworkClient::workerLoop() {
    while (m_is_running) {
        ...
        processQueue(); // Delegate request processing to a separate method.
    }
}
